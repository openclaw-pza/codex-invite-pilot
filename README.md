...
| 服务 | 在哪 |
|---|---|
| 🏪 **发卡店铺**（作者自营，下单自动发货） | **<https://pay.ldxp.cn/shop/SUPBWSJH>** |
| **Codex 一键邀请** | <https://sms.tempmail2026.xyz>（站内「Codex 一键邀请」标签页） |
| **ChatGPT / Claude 代充** | QQ 3087943900 · 微信 Claude0558（请说明来意添加） |
| **成品号** | QQ 3087943900 · 微信 Claude0558（请说明来意添加） |
| **临时邮箱**（自建，本仓库同款，可自部署） | <https://sms.tempmail2026.xyz> |
| **验证码取号** | <https://sms.tempmail2026.xyz> |

> **<https://pay.ldxp.cn/shop/SUPBWSJH> 是我本人的店**，不是第三方转卖，出了问题直接找我。
>
> - **量大价格可谈** —— 批量采购、长期合作走单独报价，不按零售价，先聊需求再定价。
> - **推荐返现 5%** —— 你把人介绍过来下单，成交后按订单金额返你 5% 现金。
>   下单前后跟我说一声就行（QQ 3087943900 · 微信 Claude0558），不用注册、也不用带参数的推广链接。
...
📮 **QQ 3087943900 · 微信 Claude0558（请说明来意添加）**
...
### 顶栏 GitHub star 按钮（可选）

买家页右上角可以挂一个 GitHub star 按钮（深色胶囊 + 图标 + 实时 star 数）。

**默认不显示**，配了才有：

```bash
GITHUB_REPO=你的用户名/你的仓库    # 留空 = 顶栏没有这个按钮
```

> 为什么默认关：页脚那行署名是许可证要求的，顶栏这个是推广位。
> 你拿这套代码去卖自己的东西，不该被默认塞一个指向别人仓库的按钮。fork 之后指向你自己的仓库就行。

计数走服务端取、缓存 30 分钟（GitHub 匿名接口每小时 60 次，所有访客共用一个出口 IP，不缓存会撞限流）。取不到就只显示图标，**不会显示成「★ 0」**——「读不到」和「读到了是 0」是两件事。
