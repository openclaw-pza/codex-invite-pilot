<!-- 草案 v2 · 2026-08-29 · 仓库名已定 codex-invite-pilot -->
<!-- 【待填】标记处只有安哥能填。填完删掉本注释块。 -->

# codex-invite-pilot

> 一套**无人值守**的端到端自动履约链路：从收到一封信开始，到一个账号完成一次真实操作为止，中间没有人。
> Node.js + SQLite，333 项测试，正在生产环境运行。
...
【待填：一段自我介绍。建议包含——你是做什么的、这套系统是从哪个真实生意里长出来的、
跑了多久、处理过多少单。真实数字比形容词有说服力。】

**这不是练手项目，是一套正在真钱运行的生产系统的完整源码。**
文档里每一条"为什么"，背后都有一次真实的损失。

### 1.2 自营业务

如果你只是想**用**而不是自己搭，下面这些是我自营的服务：

| 服务 | 说明 |
|---|---|
| **Codex 一键邀请** | 【待填：一句话 + 链接】 |
| **ChatGPT / Claude 代充** | 【待填】 |
| **成品号** | 【待填】 |
| **临时邮箱**（自建，可自部署） | 【待填：链接】 |
| **验证码取号** | 【待填：链接】 |
...
自己把系统搭起来之后，最大的成本不是代码，是**稳定的货源**。

我长期供应【待填：写你真正能稳定供的——Graph 令牌号 / 成品号 / 接码资源等】，
支持批量与长期合作。有量的直接联系。
...
```bash
git clone https://github.com/【待填】/codex-invite-pilot.git
...
**演示模式下**不调用任何外部付费服务，可以先把发卡、号池、管理后台整条走一遍再决定要不要接真实通道。
